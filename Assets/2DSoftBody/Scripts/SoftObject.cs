...
namespace SoftBody2D
{
	...
	public class SoftObject : MonoBehaviour
	{
		...
		private void UpdateJoint(SoftObjectJoint softObjectJoint, Vector3? localPosition = null, float[] weights = null)
		{
			...
			softObjectJoint.Joint.autoConfigureConnectedAnchor = AutoConfigureConnectedAnchor;
			softObjectJoint.Joint.autoConfigureDistance = AutoConfigureDistance;
			softObjectJoint.Joint.connectedBody = thisRigidbody;
			softObjectJoint.Joint.distance = Distance;
			softObjectJoint.Joint.frequency = Frequency;
			softObjectJoint.Joint.dampingRatio = DampingRatio;
			...
			if (localPosition != null)
			{
				...
				softObjectJoint.Joint.connectedAnchor = localPosition.Value;
			}
			if (weights != null)
			{
